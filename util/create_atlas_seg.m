function [median_conn, var_conn, num_samples, sem_conn] = create_atlas_seg(all_conn, all_roi, all_resect, region_list, band, threshold, silence_output)
...
%   var_conn (double): (i,j) matrix of variances of connectivity 
...
median_conn = NaN(length(region_list),length(region_list),num_patients);
...
for p = 1:num_patients
    ...
    for i = 1:length(region_list) % first region
        
        % get electrodes contained within first region

        if isempty(resect_boolean)
            first_reg_elec = (patient_electrode_regions == region_list(i));
        else  
        ...
        for j = 1:length(region_list) % second region (used to be (i+1):90)
            
            % get electrodes contained within second region
            if isempty(resect_boolean)
                second_reg_elec = (patient_electrode_regions == region_list(j));
            else
...
var_conn(num_samples < threshold) = NaN;
